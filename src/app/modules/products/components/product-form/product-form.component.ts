...
export class ProductFormComponent implements OnInit, OnDestroy{
  ...
  handleSubmitAddProduct(): void {
    if (
      this.addPrductForm?.value &&
      this.addPrductForm?.valid
    ) {
  ...
  getProductSelectedDatas(productId: string): void {
    const allProdcuts = this.productAction.productDatas;
    if (allProdcuts.length > 0) {
      const  productFiltered = allProdcuts.filter(
